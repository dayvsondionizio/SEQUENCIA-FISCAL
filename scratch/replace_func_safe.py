...
# Replace all occurrences of name.split(/[/\\]/) with a safer alternative
new_function = r"""
        ...
        try {
          ...
          for (const name of Object.keys(zip.files)) {
            ...
            if (!name.toLowerCase().endsWith('.zip') && !name.toLowerCase().endsWith('.rar')) {
              ...
              const innerArchiveName = name.replace(/\\/g, '/').split('/').pop() || name;
...
with open(file_path, 'w', encoding='utf-8') as f:
    f.write(new_content)
print("Function replaced successfully with SAFER regex-free split!")
